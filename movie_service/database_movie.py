...
class Movie(db.Model):
    ...
    ** The Movie table holds information for a given movie
      ...
      movie_rating: indicates the rating of the movie [1=G, 2=PG, 3=PG13, 4=R]
...
class GenreOfMovie(db.Model):
    ...
    genre_id = db.Column(db.Integer, db.ForeignKey(""), nullable=False)
...
class ActsIn(db.Model):
    movie_id = db.Column(db.Integer, db.ForeignKey(""), nullable=False)
    actor_id = db.Column(db.Integer, db.ForeignKey(""), nullable=False)
...
class DirectorOf(db.Model):
    movie_id = db.Column(db.Integer, db.ForeignKey(""), nullable=False)
    director_id = db.Column(db.Integer, db.ForeignKey(""), nullable=False)
...
class StudioOf(db.Model):
    movie_id = db.Column(db.Integer, db.ForeignKey(""), nullable=False)
    studio_id = db.Column(db.Integer, db.ForeignKey(""), nullable=False)
...
def init_genre_table():
    ...
    db.session.add(Genre(genre_id=5, genre_name="Crime"))
    db.session.add(Genre(genre_id=6, genre_name="Drama"))
    db.session.add(Genre(genre_id=7, genre_name="Fantasy"))
    db.session.add(Genre(genre_id=8, genre_name="Fiction"))
    db.session.add(Genre(genre_id=9, genre_name="Horror"))
    db.session.add(Genre(genre_id=10, genre_name="Mystery"))
    db.session.add(Genre(genre_id=11, genre_name="Political"))
    db.session.add(Genre(genre_id=12, genre_name="Romance"))
    db.session.add(Genre(genre_id=13, genre_name="Thriller"))
    db.session.add(Genre(genre_id=14, genre_name="SciFi"))
    db.session.commit()
...
def init_studio_table():
    ...
    db.session.add(Studio(studio_id=2,studio_name="Sony Pictures"))
